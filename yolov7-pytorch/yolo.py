...
class YOLO(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            #print(label, top, left, bottom, right,(top+bottom)/2-1,(left+right)/2-1,(top+bottom)/2+1,(left+right)/2+1)
            ...
            draw.rectangle(((left+right)/2+2,(top+bottom)/2+2,(left+right)/2-2,(top+bottom)/2-2), fill="#FF0000", outline="#FF0000", width=3)
            draw.text(text_origin, str(label,'UTF-8'), fill=(0, 0, 0), font=font)
            self.center.append([(left+right)/2,(top+bottom)/2])
            del draw
        # if len(center) == 0:
        #     return image,0
        # else:
...
class YOLO_ONNX(object):
    _defaults = {
        ...
        "classes_path"      : 'model_data/coco_classes copy.txt',
        #---------------------------------------------------------------------#
        #   anchors_path代表先验框对应的txt文件，一般不修改。
        #   anchors_mask用于帮助代码找到对应的先验框，一般不修改。
        #---------------------------------------------------------------------#
